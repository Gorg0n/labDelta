...
data_meteo = pd.read_csv('TutorialInput/DataMeteo/datameteo.csv', delimiter=',')
    ## quarter-hour consumption curves
load_el = pd.read_csv('TutorialInput/user_qc_kW_el.csv', delimiter=';')


# define users
school = User(id='school', dem=load_el['school'], pod='None', group='pubblic institue',
              plants='pv1', carrier=['electricity'])
pmi = User(id='pmi', dem=load_el['pmi'], pod='None', group='pmi', plants='pv2',
           carrier=['electricity'])
...
inverter2 = AuxiliaryComponent(id='inverter1', tech='inverter', replacement_year=10, replacement_cost=140)

# define production system


pv1 = PvPanels(id='pv1', pod=None, ma=None, ts=None, tech='pv', user='school', status='new', power=100,
               gse_mode='rid',
               ...
               mu_voc_ref=0.24, ser_cell=60, t_cell_noct_c=44, area=1.81, n_series=16, n_parallel=16,
               carrier='electricity', dur_inc_kWh=0, inc_kWh=0, inc_kW=0, aux_components=[inverter1], oem_cost_kWh=0)



pv2 = PvPanels(id='pv2', pod=None, ma=None, ts=None, tech='pv', user='pmi', status='new', power=500,
               gse_mode='rid',
               decay=0.006, life_time=20, p_con=1, cost_kW=1500, oem_cost_kW=40, inc=0, dur_inc=10, mode_mppt=1,
               isc_ref=11.32,
               voc_ref=43.8, t_cell_ref_c=25, I_tot_ref=1000,
               vmppt_ref=37.2, imppt_ref=10.76, mu_isc_ref=0.04,
               mu_voc_ref=0.24, ser_cell=60, t_cell_noct_c=44, area=1.81, n_series=36, n_parallel=36,
...
prosumer2 = Prosumer(id='pmi', plant=[pv2], user=[pmi], list_carrier=['electricity'])

# calculate prosumer energy perfomance without REC
out_en_prosumer1 = prosumer1.energy_perfomance(time=time)
out_en_prosumer2 = prosumer2.energy_perfomance(time=time)


# calculate prosumer economical perfomance without REC

out_ec_prosumer2 = prosumer2.economic_perfomance(time=time, t_inv=20, down_payment_percentual=100, t_res=0,
                                                 int_rate=0.03,
                                                 pr_import={'electricity': 320, 'heat':0},
                                                 pr_export={'electricity': 320*0.8, 'heat': 0}, tax=20)
...
# define Rec
rec = Rec(id='Rec', prosumer=[prosumer1, prosumer2], consumer=user_potential, rec_plant=[], list_carrier=['electricity'])
...
list_prosumer=[prosumer1,prosumer2]
